...
function search(method, params) {
  let url = `http://localhost:5050/search?method=${encodeURIComponent(method)}`;
...
const searchInputsDiv = document.querySelector("#search-inputs");

searchMethod.onchange = function () {
  const methodValue = searchMethod.value;
  searchInputsDiv.innerHTML = "<input placeholder='select a search method'/>";

  switch (methodValue) {
    case "firstname":
      searchInputsDiv.innerHTML =
        "<input placeholder='enter a first name' id='firstname-input' />";
      break;

    case "lastname":
      searchInputsDiv.innerHTML =
        "<input placeholder='enter a last name' id='lastname-input' />";
      break;
    ...
    case "userid":
      searchInputsDiv.innerHTML =
        "<input placeholder='enter a username id name' id='userid-input' />";
      break;

    default: // shouldn't really be called
      alert("Unknown search method selected");
  }
};
...
searchBtn.onclick = function () {
  ...
  console.log("Search method:", methodValue);
  ...
  switch (methodValue) {
    case "firstname":
      ...
      const firstname = document
        ...
        ?.value.trim();
      if (!firstname) {
        alert("Please enter a firstname before searching!");
        return;
      }
      params = { firstname };
      break;
    ...
    case "lastname":
      ...
      if (!lastname) {
        alert("Please enter a lastname before searching!");
        return;
      }
      params = { lastname };
      break;
    ...
    case "firstname-lastname":
      const fullname_firstname = document
        .querySelector("#firstname-input")
        ?.value.trim();
      const fullname_lastname = document
        .querySelector("#lastname-input")
        ?.value.trim();
      if (!fullname_firstname || !fullname_lastname) {
        alert("Please enter a firstname and lastname before searching!");
        return;
      }
      params = { firstname: fullname_firstname, lastname: fullname_lastname };
      break;

    case "userid":
      const userid = document.querySelector("#userid-input")?.value.trim();
      if (!userid) {
        alert("Please enter an username id before searching!");
        return;
      }
      params = { userid };
      break;
...
function loadHTMLTable(data) {
  ...
  let tableHtml = "";
  data.forEach(function ({ username, firstname, lastname }) {
    tableHtml += "<tr>";
