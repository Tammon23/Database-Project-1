...
class DbService {
  ...
  // ----------------------- NEW -----------------------
  async registerNewUser(username, firstName, lastName, dateOfBirth, password) {
    ...
    try {
      ...
      const insertId = await new Promise((resolve, reject) => {
        const query =
          "INSERT INTO users (username, password, firstname, lastname, date_of_birth, registerday, signintime) VALUES (?, ?, ?, ?, ?, ?, NULL);";
  ...
  async LogInUser(username, password) {
    ...
    if (response.length > 0) {
      ...
      // determine if the hashed password matches the given password
      const isValid = await bcrypt.compare(password, response[0].password);

      if (isValid) {
        // update signinn time
        const updateQuery =
          "UPDATE users SET signintime = NOW() WHERE username = ? LIMIT 1;";

        await new Promise((resolve, reject) => {
          connection.query(updateQuery, [username], (error, result) => {
            if (error) reject(new Error(error.message));
            else resolve(result);
          });
        });
  ...
  async searchByFirstname(firstname) {
    try {
      const response = await new Promise((resolve, reject) => {
        const query =
          "SELECT username, firstname, lastname FROM users where firstname = ?;";
  ...
  async searchByLastname(lastname) {
    try {
      const response = await new Promise((resolve, reject) => {
        const query =
          "SELECT username, firstname, lastname FROM users where lastname = ?;";
        connection.query(query, [lastname], (error, results) => {
          if (error) reject(new Error(error.message));
          else resolve(results);
        });
      });
  ...
  async searchByFirstnameLastname(firstname, lastname) {
    try {
      const response = await new Promise((resolve, reject) => {
        const query =
          "SELECT username, firstname, lastname FROM users where firstname = ? AND lastname = ?;";
